...
typedef int (*gpl_for_each_fn2)(void *, void *);
...
public:
    ...
    int push_front(void **pPointer, int n);
...
public:
    ...
    int push_front(const TPointerList &rhs)
    {
        return GPointerList::push_front((void **)rhs.begin(), rhs.size());
    }
    ...
    const_iterator find(const void *obj, int (*compare)(const T *, const void *)) const
    {
        for (const_iterator iter = begin(); iter < end(); ++iter)
            if ((*compare)(*iter, obj) == 0)
                return iter;
        return end();
    }
    ...
    int for_each2(iterator beg, iterator end, gpl_for_each_fn2 fn, void *param)
    {   return GPointerList::for_each2((void **)beg, (void **)end, fn, param);    }
};
