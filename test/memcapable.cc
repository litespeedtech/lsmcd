...
#define SASL_ENV_USERNAME "SASL_USERNAME"
#define SASL_ENV_PASSWORD "SASL_PASSWORD"
#endif
...
#define verify(expression) \
    do { \
        if (ensure(expression, #expression, __FILE__, \
                    __LINE__) == TEST_FAIL)    \
        {    \
            fprintf(stderr, "VERIFY FAIL IN MEMCAPABLE AT line #%d\n", __LINE__);  \
            return TEST_FAIL;  \
        }    \
    } while (0)
#define execute(expression) \
    do {    \
        if (ensure(expression == TEST_PASS, #expression, __FILE__, \
            __LINE__) == TEST_FAIL) \
        {   \
            fprintf(stderr, "EXECUTE FAIL IN MEMCAPABLE AT %d\n", __LINE__);  \
            return TEST_FAIL; \
        }   \
    } while (0)
                                                    ...
                                                    uint8_t cc, uint16_t status)
{
  ...
  if ((rsp->plain.message.header.response.opcode != cc) &&
      (rsp->plain.message.header.response.opcode == PROTOCOL_BINARY_CMD_SASL_LIST_MECHS))
      // They used LIST_MECHs as a NOOP.  EVIL!  Allow it here!
      cc = rsp->plain.message.header.response.opcode;
  if (rsp->plain.message.header.response.opcode != cc) 
      fprintf(stderr, "Unexpected response opcode: 0x%x != 0x%x\n",
  ...
  if (status == PROTOCOL_BINARY_RESPONSE_SUCCESS)
  {
    ...
    case PROTOCOL_BINARY_CMD_SASL_STEP:
      break; // won't bother with any verification for now.
...
#ifdef SASL_ON
bool s_did_auth = false;
#define SASL_CHECK  if ((!s_did_auth) && (sasl_authenticate() == TEST_FAIL)) \
                        return TEST_FAIL; \
                    else s_did_auth = true;
#else
#define SASL_CHECK
#endif
                             ...
                             unsigned int *len)
{
    ...
    {
        fprintf(stderr, "SASL_GET_USERNAME callback FAILED!\n");
                             ...
                             sasl_secret_t **psecret)
{
    ...
    {
        fprintf(stderr, "SASL password callback failed!\n");
        return SASL_BADPARAM;
    }
...
static enum test_return sasl_authenticate(void) 
{
    command cmd;
    response rsp;
    ...
    const char *username;
    const char *password;
    bool useSasl = ((username = getenv(SASL_ENV_USERNAME)) &&
                    (password = getenv(SASL_ENV_PASSWORD)));
    if (!useSasl)
        return TEST_PASS;
    
    raw_command(&cmd, PROTOCOL_BINARY_CMD_SASL_LIST_MECHS, NULL, 0, NULL, 0);
    execute(send_packet(&cmd));
    execute(recv_packet(&rsp));
    verify(validate_response_header(&rsp, PROTOCOL_BINARY_CMD_SASL_LIST_MECHS,
                                    PROTOCOL_BINARY_RESPONSE_SUCCESS));
  
    sasl_conn_t *conn;
    int ret;
    char chSecret[256];
    sasl_secret_t *secret = (sasl_secret *)chSecret;
    secret->len = strlen(password);
    strcpy((char *)secret->data, password);

    sasl_callback_t callbacks[] = {
        { SASL_CB_USER, (int (*)())sasl_get_username, (void *)username },
        { SASL_CB_AUTHNAME, (int (*)())sasl_get_username, (void *)username },
        { SASL_CB_PASS, (int (*)())sasl_get_password, (void *)secret },
        { SASL_CB_LIST_END, NULL, NULL } };
        
    ret = sasl_client_init(NULL);
    if (ret != SASL_OK)
    {
        fprintf(stderr, "SASL - sasl_client_init failed %s\n", 
                sasl_errstring(ret, NULL, NULL));
        return TEST_FAIL;
    }
    ret = sasl_client_new("memcached", 
                          "127.0.0.1",
                          NULL,         // ip port local
                          NULL,         // ip port remote
                          callbacks,    // callbacks
    ...
    if (ret != SASL_OK)
    {
        fprintf(stderr, "SASL - sasl_client_new failed %s\n", 
                sasl_errstring(ret, NULL, NULL));
        return TEST_FAIL;
    }
    // From here on don't forget to close the connection on exit
    const char *mech_used;
    const char *client_out;
    unsigned int client_len;
    ret = sasl_client_start(conn, 
                            "PLAIN",    // mech
                            NULL,       // prompt_need
                            &client_out,// client out data
                            &client_len,// client out len
                            &mech_used);
    if ((ret != SASL_OK) && (ret != SASL_CONTINUE))
    {
        fprintf(stderr, "sasl_client_start failed %s\n", 
                sasl_errstring(ret, NULL, NULL));

        sasl_dispose(&conn);
        return TEST_FAIL;
    }
    raw_command(&cmd, PROTOCOL_BINARY_CMD_SASL_AUTH, "PLAIN", 5, client_out, 
                client_len);
    execute(send_packet(&cmd));
    execute(recv_packet(&rsp));
    if (rsp.plain.message.header.response.status != PROTOCOL_BINARY_RESPONSE_SUCCESS)
    {
        fprintf(stderr, "SASL Validation failed\n");
        sasl_dispose(&conn);
        return TEST_FAIL;
    }
    verify(validate_response_header(&rsp, PROTOCOL_BINARY_CMD_SASL_AUTH,
                                    PROTOCOL_BINARY_RESPONSE_SUCCESS));
    /*
    printf("sasl_client_step, bodylen: %d\n", rsp.plain.message.header.response.bodylen);
    ret= sasl_client_step(conn, 
                          rsp.bytes + 28,
                          rsp.plain.message.header.response.bodylen,
                          NULL, &client_out, &client_len);

    if (ret != SASL_OK && ret != SASL_CONTINUE)
    {
        printf("sasl_client_step failed: %s\n",sasl_errstring(ret, NULL, NULL));
        sasl_dispose(&conn);
        return TEST_FAIL;
    }
    printf("sasl step\n");
    raw_command(&cmd, PROTOCOL_BINARY_CMD_SASL_STEP, "PLAIN", 5, client_out, client_len);
    execute(send_packet(&cmd));
    execute(recv_packet(&rsp));
    verify(validate_response_header(&rsp, PROTOCOL_BINARY_CMD_SASL_STEP,
                                    PROTOCOL_BINARY_RESPONSE_SUCCESS));
    */
    /* Release resources */
    sasl_dispose(&conn);
...
/* Server closes the connection too fast so this test is commented out for now
...
struct testcase testcases[]= {
  ...
  //{ "binary quit", test_binary_quit }, // The server closes the connection too fast
  ...
  while ((cmd= getopt(argc, argv, "qt:vch:p:PT:?ab")) != EOF)
  {
    ...
    default:
      fprintf(stderr, 
              ...
              "\t" SASL_ENV_USERNAME " must be set to the user for SASL checking\n"
              "\t" SASL_ENV_PASSWORD " must be set to the password for SASL checking\n"
#endif              
              ,argv[0]);
      return EXIT_SUCCESS;
    }
...
#ifdef SASL_ON
  if (getenv(SASL_ENV_USERNAME))
      fprintf(stdout, "ASCII commands disabled with SASL enabled\n");
#endif
...
#ifdef SASL_ON              
    if ((getenv(SASL_ENV_USERNAME)) && (testcases[ii].description[0] == 'a'))
        continue;
#endif
